...
int main(int argc, char **argv)
{
    ...
    double *au = (double *) _mm_malloc((3*npos) * (3*npos) * sizeof(double), 64);// matrix for HD interactions
    double *buf   = (double *) _mm_malloc(3*npos*sizeof(double), 64);
    double *hd_vec   = (double *) _mm_malloc(3*npos*sizeof(double), 64);
...
//        traj_write(output_filename, init_label, npos, pos, types);
    ...
    for (int interval=1; interval<=num_intervals; interval++)
    {
        ...
        bd(npos, pos, buf, types, box_width, pos_copy, next, forces, f_const, au, rad, xi, nr, nk, hd_vec);
        sprintf(label, "%f %f",
